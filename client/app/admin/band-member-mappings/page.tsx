...
export default function BandMemberUsernameMappingsAdmin() {
  ...
  const fetchMappings = async () => {
    try {
      ...
      const { data, error: fetchError } = await (supabase
        ...
        .select('*') as any)
        .order('band_member_name', { ascending: true });
  ...
  const handleCreate = async () => {
    try {
      if (!formData.band_member_name || !formData.username) {
        alert('Band member name and username are required');
        return;
      }

      const insertData: MappingInsert = {
        band_member_name: formData.band_member_name,
        username: formData.username,
        notes: formData.notes || null,
      };

      const query: any = supabase.from('band_member_username_mappings');
      const { error: insertError } = await query.insert(insertData);

      if (insertError) throw insertError;

      setIsCreating(false);
      setFormData({ band_member_name: '', username: '', notes: '' });
      fetchMappings();
    } catch (err: any) {
  ...
  const handleUpdate = async (id: number) => {
    try {
      ...
      const updateData: Partial<MappingInsert> = {
        band_member_name: formData.band_member_name || mapping.band_member_name,
  ...
  const startEdit = (mapping: UsernameMapping) => {
    ...
    setFormData({
      band_member_name: mapping.band_member_name,
  ...
  const filteredMappings = mappings.filter(m =>
    m.band_member_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">
              Band Member Username Mappings
            </h1>
            <p className="text-gray-400">
              Manage how band member names link to community profiles
            </p>
          </div>
          <button
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by band member name or username..."
              value={searchTerm}
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <input
                  type="text"
                  value={formData.band_member_name}
                  onChange={(e) => setFormData({ ...formData, band_member_name: e.target.value })}
        ...
        <div className="mt-6 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
          ...
          <ul className="text-gray-300 text-sm space-y-1 list-disc list-inside">
            <li>Add mappings to link band member names to their community usernames</li>
            <li>Band member cards in /internal-bands will become clickable</li>
            <li>Multiple name variations can map to the same username</li>
            <li>Only active mappings are used for linking</li>
